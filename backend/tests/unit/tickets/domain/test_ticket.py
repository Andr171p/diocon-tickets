...
class TestAssignTo:
    ...
    def test_reassign_ticket_registers_ticket_reassigned_event(self, ticket_in_open, support_agent_id):
        ...
        first_assignee_id = uuid.uuid4()
        second_assignee_id = uuid.uuid4()

        # очистка события TicketCreated, которое появилось при создании тикета
        list(ticket_in_open.collect_events())

        # первое назначение, нужно только подготовки состояния
        # у тикета должен появиться страый испольнитель
        ticket_in_open.assign_to(
            assignee_id=first_assignee_id,
            assigned_by=support_agent_id,
        )

        # очистка события первого назначения
        list(ticket_in_open.collect_events())
        ...
        events = list(ticket_in_open.collect_events())
