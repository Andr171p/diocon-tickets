...
class TestProjectCreate:
    def test_create_should_succeed_with_valid_data(self, project_data):
        ...
        assert project.participants[0].user_id == project_data["owner_id"]
        assert project.participants[0].project_role == ProjectRole.OWNER
        assert project.participants[0].added_by == project_data["created_by"]
...
class TestProjectParticipants:
    def test_add_participant_should_succeed_when_called_by_owner(
        ...
        created_project.add_participant(
            user_id=manager_id,
            project_role=ProjectRole.MANAGER,
            added_by=owner_id,
            added_by_role=UserRole.CUSTOMER_ADMIN,
        )
        excepted_participants_count = 2

        assert len(created_project.participants) == excepted_participants_count
        added = next(
            participant for participant in created_project.participants
            if participant.user_id == manager_id
        )
        assert added.project_role == ProjectRole.MANAGER
        assert added.added_by == owner_id

    def test_add_participant_should_raise_error_when_user_already_participant(
        self, created_project, owner_id
    ):
        with pytest.raises(InvariantViolationError, match="is already a participant"):
            created_project.add_participant(
                user_id=owner_id,
    ...
    def test_add_participant_should_raise_permission_error_when_not_owner_or_manager(
        ...
        with pytest.raises(
            PermissionDeniedError, match="Only owner or support stuff can add participants"
        ):
            created_project.add_participant(
                user_id=viewer_id,
